...
import { TReceiptBody } from "./types";

export const ReceiptBody = React.forwardRef(
	(props: TReceiptBody, ref: React.LegacyRef<HTMLDivElement>) => {
		return (
			<div ref={ref}>
				<ContainerReceipt>
					<Top>
					...
					<Body>
						<h2>**RECIBO**</h2>
						<h3>{`Valor: ${
							props.receiptDataInput.value !== ""
								? parseInt(props.receiptDataInput.value)
										.toFixed(2)
										.replace(".", ",")
								: "0,00"
						}`}</h3>
						<p>{`Recebi(emos) de ${props.receiptDataInput.name.toUpperCase()} - ${mCPF(
